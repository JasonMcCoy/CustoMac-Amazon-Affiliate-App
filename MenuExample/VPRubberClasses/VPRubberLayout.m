...
- (void)prepareLayout {
    ...
    NSArray *noLongerVisibleBehaviours = [self.dynamicAnimator.behaviors filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIAttachmentBehavior *behaviour, NSDictionary *bindings) {
        UICollectionViewLayoutAttributes *attrib = [[behaviour items] lastObject];
        BOOL currentlyVisible = [itemsIndexPathsInVisibleRectSet member:[attrib indexPath]] != nil;
        return !currentlyVisible;
    }]];
    
    [noLongerVisibleBehaviours enumerateObjectsUsingBlock:^(id obj, NSUInteger index, BOOL *stop) {
        [self.dynamicAnimator removeBehavior:obj];
        UICollectionViewLayoutAttributes *attrib = [[obj items] lastObject];
        
        [self.visibleIndexPathsSet removeObject:[attrib indexPath]];
    }];
